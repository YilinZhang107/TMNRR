...
def corrected_loss(corrected_Y, alpha, indexes, error_index, device):
    ...
    label = torch.Tensor(corrected_Y).to(device)

    error_mask = torch.isin(indexes, error_index).to(device)

    noise_loss = None
    if error_mask.any():
        noise_loss = torch.sum((label[error_mask] - alpha[error_mask] / S[error_mask]) ** 2, dim=1, keepdim=True)
    return noise_loss, error_mask.sum() > 0

# Adjusted cross-entropy loss based on the Dirichlet distribution
def ce_loss(label, alpha, c, global_step, annealing_step, device, indexes, error_index, l2):
    S = torch.sum(alpha, dim=1, keepdim=True)
    E = alpha - 1
...
def similarity_loss(S, near_indices, matrix, indexes, device):
