...
def log_metrics(set_name, metrics, args, logger=None):
    """Log training/validation metrics."""
    if logger is None:
        print("{}: Loss: {:.5f} | Acc: {:.5f}".format(set_name, metrics["loss"], metrics["acc"]))
    else:
        if args.task_type == "multilabel":
            logger.info(
...
class History(object):
    """History class for tracking model performance over training."""
    ...
    def get_target_margin(self, data_idx1, data_idx2):
        """Get target and margin for ranking loss."""
